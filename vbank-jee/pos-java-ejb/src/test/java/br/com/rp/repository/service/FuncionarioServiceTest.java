...
public class FuncionarioServiceTest extends AbstractTest {
	...
	public void deveAlterarFuncionarioComSucesso() {
		...
		funcionario.setNome("monica");
		funcionarioService.save(funcionario);
		Assert.assertEquals("monica", funcionarioService.findById(100001L).getNome());
	}
	...
	public void deveCompararNomeFuncionario() {
		Assert.assertEquals("formiga", funcionarioService.findById(100001L).getNome());
	}
	...
	public void deveRetornarFuncionarioPeloCpf() {
		Assert.assertEquals("monica", funcionarioService.findByCpf("09005548625").getNome());
	}
